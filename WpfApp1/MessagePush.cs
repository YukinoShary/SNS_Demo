...
namespace WpfApp1
{
    class MessagePush
    {
        private static ConcurrentDictionary<string, int> messageCounters = new ConcurrentDictionary<string, int>();         //每个聊天对象的未读消息数(线程安全字典)
        ...
        private static void SingleMessage(string type,string source,string content)
        {
            ...
            {
                messageCounters.TryAdd(source, 1);
        ...
        private static void GroupMessage(string type, string source, string content,string target)
        {
            ...
            if (type.Equals("Text")||type.Equals("Picture"))
            {               
                r.Content = content;
            }
            else
